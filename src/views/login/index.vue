...
export default {
  ...
  data() {
    ...
    return {
      ...
      loginRules: {
        mobile: [
          { required: true, message: '账号名不能为空', trigger: ['change', 'blur'] },
          { validator: validateMobile, trigger: ['change', 'blur'] }
        ],
        password: [
          { required: true, message: '密码名不能为空', trigger: ['change', 'blur'] },
          { min: 6, max: 12, message: '密码只能在6-12位', trigger: ['change', 'blur'] }
        ]
      },
      loading: false,
  ...
  methods: {
    ...
    handleLogin() {
      ...
      this.$refs.loginForm.validate((flag) => {
        this.getToken(this.loginForm).then((res) => {
          console.log(res, 777)
          this.loading = false
